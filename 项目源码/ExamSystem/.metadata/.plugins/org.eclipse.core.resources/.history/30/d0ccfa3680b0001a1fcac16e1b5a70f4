...
public class UserDao implements IUserDao {
	...
	public Sysuser login(Sysuser user) {
		...
		List<Sysuser> list = DBUnitHelper.executeQuery(sql, Sysuser.class, user.getUsername(), user.getUserpwd());
		if (list.size() > 0) {
			user = list.get(0);
		} else {
			user = null;
	...
	public List<SysFunction> initpage(Sysuser user) {
		...
		if (user.getRoleid().equals(-1)) {
			...
			list = DBUnitHelper.executeQuery(sql, SysFunction.class, user.getRoleid());
		}
		return list;
	}
	...
	public Pager<Sysuser> list(Sysuser user, PageControl pc) {
		...
		String sql = "SELECT USERID,ROLEID,USERNAME,USERPWD,USERTRUENAME,USERSTATE FROM " + " SYSUSER WHERE USERID>0 ";
	...
	public Integer add(Sysuser user) {
		...
		return DBUnitHelper.executeUpdate(sql, user.getRoleid(), user.getUsername(), user.getUserpwd(),
				user.getUsertruename(), user.getUserstate());
	}
	...
	public Integer edit(Sysuser user) {
		// TODO Auto-generated method stub
		String sql = "UPDATE SYSUSER SET ROLEID=?,USERNAME=?," + "USERPWD=?,USERTRUENAME=?,USERSTATE=? WHERE USERID=?";
		Integer rtn = DBUnitHelper.executeUpdate(sql, user.getRoleid(), user.getUsername(), user.getUserpwd(),
				user.getUsertruename(), user.getUserstate(), user.getUserid());
		return rtn;
	}
