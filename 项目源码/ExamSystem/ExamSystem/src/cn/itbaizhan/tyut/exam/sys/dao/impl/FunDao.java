...
public class FunDao implements IFunDao {
	...
	public Integer addfun(SysFunction fun) {
		...
		Integer rtn = DBUnitHelper.executeUpdate(sql, fun.getFunname(), fun.getFunurl(), fun.getFunpid(),
				fun.getFunstate());
		return rtn;
	}
	...
	public Pager<SysFunction> list(SysFunction fun, PageControl pc) {
		String sql = "SELECT A.FUNID,A.FUNNAME,A.FUNURL,A.FUNPID,A.FUNSTATE,"
				+ "(CASE WHEN B.FUNNAME IS NULL THEN '无' ELSE B.FUNNAME END) AS FUNPNAME" + " FROM SYSFUNCTION A "
	...
	public Integer edit(SysFunction fun) {
		...
		Integer rtn = DBUnitHelper.executeUpdate(sql, fun.getFunname(), fun.getFunurl(), fun.getFunstate(),
				fun.getFunid());
		return rtn;
	}
