...
public class RoleDao implements IRoleDao {
	...
	public Pager<Sysrole> list(Sysrole role, PageControl pc) {
		...
		String sql = "SELECT ROLEID,ROLENAME,ROLEDESC,ROLESTATE FROM " + " SYSROLE WHERE ROLEID>-2 ";
	...
	public Integer add(Sysrole role) {
		...
		String sql = "INSERT INTO SYSROLE(ROLENAME,ROLEDESC,ROLESTATE) " + "VALUES (?,?,?)";
	...
	public List<SysFunction> initfunlist(Sysrole role) {
		...
		String sql = "select A.funid,A.funname,A.funurl,A.funpid,A.funstate, "
				...
				+ "LEFT OUTER JOIN ROLERIGHT B ON A.FUNID=B.FUNID AND B.ROLEID=? " + "WHERE A.FUNSTATE=1  ";
	...
	public Sysrole detail(Sysrole role) {
		String sql = "SELECT ROLEID,ROLENAME,ROLEDESC,ROLESTATE FROM " + " SYSROLE WHERE ROLEID=? ";
		List<Sysrole> list = DBUnitHelper.executeQuery(sql, Sysrole.class, role.getRoleid());
		return list.get(0);
	}

	public ArrayList<Sysrole> getALL() {
		String sql = "SELECT ROLEID,ROLENAME,ROLEDESC,ROLESTATE FROM " + " SYSROLE";
	...
	public Integer edit(Sysrole role) {
		...
		Integer rtn = DBUnitHelper.executeUpdate(sql, role.getRolename(), role.getRolestate(), role.getRoledesc(),
				role.getRoleid());
		return rtn;
	}
